    ...
    value: function onScaleChange(e) {
      ...
      var recursion = function recursion(element) {
        element.childNodes.forEach(function (node) {
          // все дочерние элем(приходят как array)
          if (node.nodeName === '#text' && node.nodeValue.replace(/\s+/g, '').length > 0) {
            // ищем текстовые узлы и не пустые текст элементы -> обращаемся к родителю -> узнаем какой font size -> умнажаем на scale
            if (!node.parentNode.getAttribute('data-fz')) {
              ...
              //user has already changed scale
              node.parentNode.style.fontSize = node.parentNode.getAttribute('data-fz') * _this2.scale + "px";
            }
          } else {
            recursion(node);
